...
const CourseViewer = ({ course: initialCourse, onBack }) => {
    ...
    const handleLessonClick = (lesson) => {
        ...
        const fullLesson = course.modules
            ?.flatMap(m => m.lessons || [])
            ?.find(l => l._id === lesson._id) || lesson;
    ...
    if (activeLesson && course) {
        return (
            <LessonViewer
                ...
                onLessonComplete={() => { setActiveLesson(null); fetchCourse(); }}
            />
        );
    }

    const percent = course?.percent ?? initialCourse.percent ?? 0;
    ...
    return (
        <div className="space-y-6">
            ...
            {!loading && errorMsg && (
                <div className="text-center py-10 bg-red-50 border border-red-200 rounded-xl px-6">
                    <p className="text-red-600 font-bold mb-2">Failed to load course content</p>
                ...
                <div className="space-y-8 pb-20">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                            <div className="flex-1">
                                ...
                                <p className="text-slate-500 leading-relaxed max-w-2xl">
                                    {course.description || 'Master the core concepts through our interactive visual learning system.'}
                                </p>
                            </div>
                            <div className="w-full md:w-64 bg-slate-50 border border-slate-100 rounded-lg p-4">
                                ...
                                <p className="text-xs font-medium text-slate-500">
                                    {initialCourse.completedLessons ?? 0} of {initialCourse.totalLessons ?? 0} lessons completed
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {course.modules?.length === 0 && (
                            <p className="text-sm text-slate-400 italic text-center py-8">No modules added yet.</p>
                        )}

                        {course.modules?.map((mod, mi) => {
                            const isOpen = !!expandedModules[mod._id];
                            const lessonsDone = mod.lessons?.filter(l => isLessonCompleted(l)).length || 0;
                            const modPercent = mod.lessons?.length > 0
                                ? Math.round((lessonsDone / mod.lessons.length) * 100)
                                : 0;

                            return (
                                <div key={mod._id} className="bg-white border border-slate-200 rounded-xl overflow-hidden">
                                    <button
